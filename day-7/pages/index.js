...
export default function Home() {
  ...
  const addGift = (e) => {
    e.preventDefault();
    if (!value) {
      return alert("No puedes agregar un regalo vacío");
    }
    setGift([...gift, value]);
    setValue("");
  };
  ...
  return (
    <main className="flex flex-col h-screen justify-center items-center align-center bg-[url('../public/christmas.png')] font-mono text-white">
      <article className="bg-green-600 space-y-5 p-5 border-2 border-white rounded-xl">
        <h1 className="font-sans text-4xl text-center">Adviency Dia 7</h1>
        ...
        <ul>
          {!gift.length ? (
            <p className="text-center">No hay un solo regalo!</p>
          ) : (
            gift.map((item, index) => (
              <li
                key={index}
